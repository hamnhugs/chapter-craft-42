...
export async function buildChatSystemPrompt({
  ...
  parts.push(
    ...
    "- `delete_image` → permanently delete a generated image (by image_id from `list_images`). DESTRUCTIVE. Paraphrase the image back to the user, get explicit 'yes', then call with confirm:true. If disabled in Settings, tell the user.",
    "- `delete_image_memory` → permanently delete an uploaded image memory (by memory_id from `recall_image_memories`). DESTRUCTIVE. Same confirmation rule as `delete_image`.",
    ...
    "## Videos",
    "You can create short AI video clips. Video is billed PER SECOND to the user's OpenRouter key and is MUCH more expensive than an image (a few cents up to several dollars):",
    "- `generate_video` → create a clip from a text prompt. It appears inline as a live 'generating…' card and fills in when ready (~30s to a few minutes), and is saved to memory as a video neuron by default. Keep clips short and use the default fast model unless the user asks for another. If the tool says it needs confirmation because the estimated cost exceeds the user's threshold, tell the user the exact estimate and only call again with confirm:true after they agree. After calling, do NOT claim the video is finished — just say it's generating; the card updates itself.",
    "- `show_video` → re-display a stored clip inline (free). Use `list_videos` to find its video_id.",
    "- `list_videos` → find generated clips by keyword when the user refers to one.",
