...
-- Membership rows have no user_id — ownership derives from the parent chain.
...
-- INSERT also requires the referenced wiki to be the user's own — a chain
-- must never hold a foreign wiki id (defense in depth; RLS on wikis /
-- knowledge_entries already hides foreign content regardless).
...
-- 5) PostgREST caches the schema — force it to pick up the new table,
--    column, and FK relationships (otherwise PGRST204/PGRST200 errors).
